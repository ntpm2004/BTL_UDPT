...
app = Flask(__name__)
db = PupDB("products_worker2.json")
...
def sync_from_master():
    ...
    if not data:
        print("[Worker 2] Nhận sync rỗng (bỏ qua).")
        return jsonify({"status": "empty"}), 200

    for pid, product in data.items():
        db.set(pid, product)
    print(f"[Worker 2] Đã nhận và lưu {len(data)} sản phẩm từ master")
...
def delete_sync():
    ...
    if pid in db.keys():
        db.remove(pid)
        print(f"[Worker 2] Đã xóa sản phẩm {pid} theo lệnh từ master")
    return jsonify({"status": "ok"}), 200
...
def process_task(action, value, subset):
    ...
    print("[Worker 2] Hoàn tất — gửi kết quả về master...")

    try:
        res = requests.post(MASTER_URL, json={"data": updated_data})
        if res.status_code == 200:
            print("[Worker 2] Đồng bộ với master thành công")
        else:
            print(f"[Worker 2] Lỗi khi đồng bộ với master: {res.text}")
    except Exception as e:
        print(f"[Worker 2] Không thể kết nối master: {e}")
